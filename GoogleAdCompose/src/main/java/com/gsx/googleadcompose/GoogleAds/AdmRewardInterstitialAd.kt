...
/**
 * Rewarded Interstitial ad (GMA Next-Gen SDK) — **one-shot** [RewardedInterstitialAd.load]: load đúng
 * 1 ad, giữ sẵn, show 1 lần rồi bỏ. KHÔNG dùng preloader (preloader tự refill cái kế -> show 1 lần ->
 * refill không bao giờ show -> phí matched request -> tụt show rate). Log tag `GoogleAds/RewardInter`.
 *
...
class AdmRewardInterstitialAd internal constructor() : RewardedInterstitialAdEventCallback {
     ...
     * [index] trong [AdmConfigAdId.listRewardInterstitialAdUnitID]. Đã có ad / đang load -> bỏ qua.
     */
    fun load(index: Int = -1, customIds: List<String>? = null) {
        ...
        RewardedInterstitialAd.load(AdRequest.Builder(unit).build(), object : AdLoadCallback<RewardedInterstitialAd> {
            override fun onAdLoaded(ad: RewardedInterstitialAd) {
                loading = false
                ad.adEventCallback = this@AdmRewardInterstitialAd
                loadedAd = ad
                log("loaded ${tag()}")
